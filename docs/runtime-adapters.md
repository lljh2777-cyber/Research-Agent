...
The independent Annotation request ceiling is 131,072 raw UTF-8 bytes of the Adapter's exact `JSON.stringify({ intent, idempotencyKey, approval })` body. JSON escaping, approval, and idempotency metadata count. The Adapter preflights that exact body before `fetch`, and the server applies the same ceiling to received bytes. Therefore content compliance alone does not imply request-envelope compliance; an escaping-heavy but content-valid record can fail with `limit_exceeded` without a network request.
...
- Annotation request: 131,072 raw serialized bytes, independently including JSON escaping, Patch Intent metadata, approval, and idempotency.
...
The local Action runner maps lint, ingest, X-Ray, and static-code-analysis to the existing Research Vault skills. Formal `knowledge.synthesis.write` archive requests cannot use that generic child-write path; they use the durable Runtime realization boundary described below. Process creation, credentials, filesystem permissions, and writes stay server-side. React imports neither plugin runtime code nor child-process APIs.

### Formal Knowledge Archive realization v1

The synthesis descriptor accepts only the Core/KB-owned archive input `{ operation: 'archive-annotation', sourceAnnotation: { id, path, revision }, targets }`. Runtime re-normalizes the request and the exact approval proof `{ status: 'approved', scope, sourceAnnotation, targets }`, constructs canonical key order, and compares canonical UTF-8 JSON digests. Caller property insertion order is irrelevant; any semantic scope/source/target mismatch fails with `approval_mismatch`. Folder/note authorization roots and every target also reject the complete reserved `.bioresearch/runtime` namespace, case-insensitively on the local filesystem.

After canonical approval succeeds, Runtime persists the initial `accepted` record before source reread, cancellation, or planning. The Vault journal lives at `.bioresearch/runtime/archive-realizations/v1/<scoped-sha256>.json`; its filename derives only from Vault/tool/idempotency scope. An authenticated checkpoint mirror lives outside the user-controlled Vault under the Runtime state directory (`%LOCALAPPDATA%/BioResearch OS/runtime-state` on Windows, or the platform state directory) with mode `0700` directories and `0600` files where supported. Windows ACL inheritance may be broader than POSIX modes, so operators must keep that directory private to the Runtime account. Nonterminal `planned`/`committing` checkpoints temporarily contain bounded generated Markdown needed for recovery; they are never exposed through manifest, API, UI, or logs.

The Runtime owns a stable random 256-bit HMAC key at `runtime-state/keys/archive-realization-hmac-v1.key`, created with exclusive `wx`, fsynced, and never placed in a manifest, API response, journal payload, UI surface, or log. Journal/checkpoint records store only `keyId`, monotonic `generation`, `previousMac`, and `mac`. The MAC is HMAC-SHA-256 over the exact full record except `mac`. Each transition increments generation and chains to the prior MAC; the external checkpoint anchors the newest generation, so replacing the Vault journal with an older valid record is detected. A missing journal can be restored from the authenticated checkpoint. During initial `accepted` persistence only, a journal without its not-yet-written checkpoint can initialize that checkpoint; later checkpoint absence fails closed. A changed/unreadable/missing key for an existing Vault journal disables formal archive rather than generating a replacement. Vaults synchronized to another machine therefore fail closed unless an authorized Runtime credential/state mechanism also supplies the matching key.

Exact states are `accepted -> planning -> planned -> committing -> completed|failed|cancelled`. Nonterminal records use exact keys `schemaVersion`, `kind`, `key`, `keyId`, `generation`, `previousMac`, `mac`, `requestDigest`, `approvalDigest`, `request`, `approval`, `state`, `targets`, `result`, `createdAt`, and `updatedAt`. `accepted`/`planning` contain no plan, snapshots, commits, or terminal. `planned` contains the full validated plan and snapshots but no committed evidence or terminal. `committing` permits only an ordered committed prefix and no terminal.

Before publishing a terminal event, Runtime writes a distinct authenticated terminal generation to both copies. Its exact top-level schema replaces `request` and `approval` with `binding`, which retains only schema/tool/request/run/session/idempotency identity, authorization-root scope, source Annotation tuple, and requested target order. It contains no `context` and no raw approval. Terminal target entries retain only path, intended revision and byte count, snapshot revision/existence, and committed status/revision; generated content is absent. `completed` requires all requested evidence and an exact completed Core result. `failed`/`cancelled` require a matching exact result and truthful ordered committed prefix. The chained HMAC makes compaction monotonic, rejects rollback to a pre-compaction generation, and makes terminal replay independent of the planner. Terminal generations cannot transition.

Startup composition audits both managed directories before publishing capability evidence. It bounds authenticated state to 256 active nonterminal realizations and 67,108,864 total bytes, validates exact filenames and schemas, HMAC/key identity, nested journal state, and the allowed one-copy recovery relationship. It removes orphan temporary files only when their exact generated name occurs inside those managed directories. Missing, replaced, unreadable, mixed, corrupt, or rolled-back keys/anchors disable only formal synthesis before manifest publication; they are never rotated or re-signed automatically. Every load rejects missing/extra keys, invalid HMAC/key identity, rollback/generation drift, impossible state combinations, malformed paths, partial plans, content/revision drift, invalid evidence, or a non-Core terminal result as `journal_corrupt`.

The compact terminal is the durable idempotency tombstone and remains until a future explicit retention policy removes the exact paired realization. A stuck nonterminal may retain sensitive recovery content up to the documented plan and state quotas. Manual recovery must stop Runtime, preserve both copies and the private key for diagnosis, and restore the exact authenticated set or remove one exact paired realization only after accepting loss of replay history; Runtime never bulk-cleans user files. Key loss/change fails closed. Cross-machine Vault sync likewise fails closed unless an authorized Runtime state mechanism transfers the matching private key and checkpoints.

The planner is read-only and must return exact `{ targets: [{ path, content }] }` in requested order. Runtime validates every entry before the first write: no missing/extra keys, duplicates, unrequested paths, empty/non-string content, target content above 1,048,576 UTF-8 bytes, or total plan JSON above 4,194,304 UTF-8 bytes. It persists the complete intended plan, including per-target missing/existing revision snapshots and intended content revisions, before committing.

Each commit rechecks containment and symlink/reparse safety, compares the current target to its snapshot, fsyncs a deterministic same-directory staging file, invokes the final mutation boundary, rechecks the snapshot immediately before rename, then renames. This is compare-before-rename protection within the Runtime containment/lock model; it does not claim a filesystem-native multi-file transaction. Journal/checkpoint updates use exclusive temporary files, file fsync/close, atomic rename, and best-effort directory fsync. Restart reconciliation treats a target as committed only when actual bytes match its intended revision; an actual snapshot resumes safely, and any third revision fails without overwrite. Failed/cancelled results contain the complete requested source tuple and only the ordered subset proven committed. Cancellation waits for realization quiescence; `run.completed` carries the full bounded output at `event.output`, while `run.failed`/`run.cancelled` carry it at `event.result`.

Stable Runtime errors include `approval_required`, `approval_mismatch`, `scope_denied`, `idempotency_conflict`, `revision_conflict`, `limit_exceeded`, `journal_corrupt`, `archive_failed`, and `archive_cancelled`. A failed or cancelled archive never rewrites or rolls back the already-saved source Annotation.

### Capability behavior

A configured full `local-web` manifest advertises same-origin Annotation and Action services plus their exact byte limits and per-Action capability map. `actions.synthesis` becomes true only when the composed server proves the Research Run transport, read-only strict-JSON planner, atomic-json-v1 journal storage, HMAC-SHA-256 authenticity anchor, and crash recovery are executable; target identity or a generic Action runner alone is insufficient. `vite-web`, hosted Web, unconfigured local Web, and desktop fail closed without loopback probes. Browser directory selection and Vite-hosted Provider, MCP, Research Run, and research-execution capabilities are unchanged. Desktop compatibility is preserved without adding Electron IPC.
