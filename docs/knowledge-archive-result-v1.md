...
Action terminal placement is exact and preserves the full bounded KnowledgeActionOutputV1: `run.completed` uses `event.output`; `run.failed` and `run.cancelled` use optional `event.result` so legacy terminals without partial evidence remain valid. Absence is the only legacy fallback and produces empty committed evidence. When `result` is present it must be the full exact Action output with matching identity and terminal status; malformed, partial, wrong-schema, or wrong-status objects are protocol drift and reject in both direct consumption and replay. The Action output/result JSON is bounded to 65,536 UTF-8 bytes. Provider and Knowledge Read terminal `result` semantics are unchanged. Research Run v1 adds no event or status; terminal protection remains authoritative, and replay returns the latest terminal Action value plus its cursor.
