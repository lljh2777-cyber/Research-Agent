...
`sourceAnnotation.id` is the bounded logical `AnnotationV1|V2.id`. `sourceAnnotation.path` is the exact Runtime annotation record path used for `annotations.read`: a normalized relative forward-slash Markdown path under `wiki/annotations/`, with no root/drive prefix, backslash, control character, empty segment, `.` segment, or `..` segment. Its extension is accepted case-insensitively (`.md`, `.MD`, or mixed case), and its actual path and extension case are preserved unchanged. `ANNOTATION_RECORD_PATH_MAX_LENGTH` is exactly 512 JavaScript UTF-16 code units using `String.prototype.length`, matching Runtime `MAX_PATH_LENGTH`; this is deliberately not a UTF-8 byte limit. KB validates but does not derive, trim, lowercase, or otherwise rewrite this path. `sourceAnnotation.revision` is the non-empty opaque Runtime content revision for that exact path captured for approval/concurrency, not `annotation.source.revision` (the source note revision) and not any archive-target expected/result revision; it is limited to 256 UTF-8 bytes.
...
Annotation/source reference IDs are limited to 256 UTF-8 bytes; Annotation note-source paths to 4,096 UTF-8 bytes; archive source-record paths to 512 JavaScript UTF-16 code units; annotation revisions to 256 UTF-8 bytes. `ANNOTATION_ARCHIVE_RUN_ID_MAX_BYTES` remains exactly 256 UTF-8 bytes. Reserved section markers remain rejected. LF is canonical; CRLF parses to the same normalized record.
