    ...
    <div className="model-picker" data-placement={placement} ref={rootRef}>
      ...
      {open && <div className="model-menu" role="menu" aria-label="Research model">
        ...
        <div className="model-menu-account">
          ...
          <span><strong>ChatGPT account</strong><small>{authStatus?.connected ? `${modelCatalog?.models?.length || 0} models 閻?${modelCatalog?.source || 'discovering'}` : authStatus?.unavailable ? 'Local service offline 閻?restart npm run dev' : 'Connect to discover available models'}</small></span>
      ...
      <div className="research-setup-config">
        <section className="research-config-block agent-identity-block">
          ...
          <div className="system-prompt-field">
            ...
            <small>{(config.systemPrompt || '').length} / 6000 閻?saved in this conversation snapshot</small>
          </div>
        </section>
        <section className="research-config-block">
          ...
          <div className="research-model-choice">
            <div><strong>{selectedModel.name}</strong><small>{selectedModel.provider} 閻?{selectedModel.detail || 'Research model'}</small></div>
...
function RetrievalPath({ activeStage, vaultName, topK, rerankLabel, packet, answerMode, wikilinksEnabled }) {
  ...
  const path = [
    ...
    ['Graph expansion', wikilinksEnabled ? packet ? `${retrieval?.graphExpanded || 0} one-hop result${retrieval?.graphExpanded === 1 ? '' : 's'} 閻?rerank: ${rerankLabel}` : 'waiting for a query' : 'Disabled for this conversation', packet || !wikilinksEnabled ? 'done' : 'current'],
