...
test('ResearchWorkspace keeps readable UI text and exposes EvidencePacket errors accessibly', async () => {
  ...
  assert.match(source, /embedding: \$\{embeddingLabel\}/)
  assert.match(source, /degraded: \$\{degradation\}/)
  assert.match(mainSource, /Generated with \$\{result\.model\} through .* - \$\{agentOutput\.iterations\} model pass/)
  assert.match(mainSource, /retrieveHybridEvidence\(question/)
  assert.match(mainSource, /remoteEmbeddingConsent/)
  assert.match(mainSource, /migrateRetrievalIndexV1/)
})
